...
export default function Classes() {
  ...
  // Filtro por día (derivado del campo date)
    const filtered = useMemo(() => {
        if (!selectedDay) return [];
        return items.filter((c) => dayNameEs(c.date) === selectedDay);
    }, [items, selectedDay]);
    ...
    if (loading) return <div className={styles.section}><p className={styles.emptyMessage}>Cargando…</p></div>;
    if (error)   return <div className={styles.section}><p className={styles.emptyMessage}>{error}</p></div>;
    ...
    return (
        ...
        {selectedDay === "" ? (
            <p className={styles.emptyMessage}>Selecciona un día para ver clases</p>
        ) : filtered.length === 0 ? (
            <p className={styles.emptyMessage}>No hay clases disponibles para {selectedDay}</p>
        ) : (
            <div>
            {filtered.map((c) => (
                ...
                <div className={styles.classInfo}>
                    ...
                    <span className={styles.infoValue}>{dayNameEs(c.date)}</span>
                    </div>
                    <div className={styles.infoItem}>
                ...
                <button className={styles.enrollButton} onClick={() => handleEnroll(c)}>
                    Inscribirse
                </button>
                </div>
            ))}
