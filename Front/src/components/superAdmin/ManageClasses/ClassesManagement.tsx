...
const ClassesManagement: React.FC = () => {
  ...
  const handleToggleStatus = async (id: string, currentStatus: ClassDto["status"]) => {
    const newStatus = currentStatus === "active" ? "inactive" : "active";
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/classes/${id}/status`, {
        method: "PATCH",
    ...
    <div className={styles.container}>
      ...
      <table className={styles.table}>
        ...
        <tbody>
          {classes.length === 0 ? (
            <tr>
              <td colSpan={8} className={styles.empty}>No hay clases registradas.</td>
            </tr>
          ) : (
            classes.map((cls) => (
              <tr key={cls.id}>
                ...
                <td className={styles.actions}>
                  ...
                  <button
                    ...
                    className={cls.status === "active" ? styles.deactivate : styles.activate}
                  >
        ...
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            ...
            <div className={styles.modalActions}>
              <button onClick={handleSaveEdit} className={styles.save}>Guardar</button>
              <button onClick={() => setEditingClass(null)} className={styles.cancel}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
